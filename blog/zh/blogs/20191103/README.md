...
3. 设$\{x\}$是对于加权拟阵$M=(S,L)$由定理2中所选择的S的第一个元素，则剩下问题可归结为求加权矩阵$M’=(S’，L’)$的最大权独立集问题，其中$S’=\{y|y\in S ~and~ \{x,y\}\in L\}$，$L’=\{B|B\in S\setminus\{x\}  ~and~ B\cup\{x\}\in L\}$
